...
authentication for `claude-code-action`). Repos whose default workflow token is set to
**"Read repository contents"** (the restrictive default) deny these scopes — the workflow
fails with `startup_failure` before any job runs.
...
The caller workflow's explicit `permissions` block declares the scopes it needs, but GitHub
caps those declarations at whatever the repo setting allows. Without the permissive setting,
`id-token: write` is denied regardless of what the YAML says.
