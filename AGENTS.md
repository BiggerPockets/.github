...
- Keep a workflow step's `run:` to a single command. Put shell logic in a script under
  `scripts/` and Python in a `.py` file, never inline heredocs, and pass inputs through
  the step's `env:` rather than interpolating `${{ }}` into shell. The suite runs
  `bash -n` on every script and shellcheck on the gym's.
